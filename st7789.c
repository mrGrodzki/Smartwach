...
#include "SPI.h"
 ...
 bool start_read_flag=0;

 extern uint8_t rtc_M;
 extern uint8_t rtc_H;
 extern uint16_t steeep;
...

{
    ...
    nrf_gpio_cfg_output( ST7789_RES);
   nrf_gpio_pin_write(ST7789_RES, true);

     nrf_gpio_cfg_output( ST7789_BLK);
   nrf_gpio_pin_write(ST7789_BLK, false);

     nrf_gpio_cfg_output( ST7789_DC);
   nrf_gpio_pin_write(ST7789_DC, false);
	nrf_delay_ms(250);
    ST7789_RST_Clr();
    nrf_delay_ms(52);
    ST7789_RST_Set();
...
ST7789_WriteString(1, 220, temp_str, Font_11x18, YELLOW, 0xBDF8);			//	Fill with Black.
   ...
   nrf_gpio_pin_write(ST7789_BLK, true);
}
...
void ST7789_WriteNumbers(uint16_t x, uint16_t y, int16_t number, FontDef font, uint16_t color, uint16_t bgcolor)
{
...
ST7789_UpdateDisplay()
{
  int32_t P,T, Pmm = 0; 
  //float Hp= 95000;
  bme280_read_adc(&T, &P);
  if( start_read_flag==0) 
    {
    start_height = P;
    start_read_flag=1;
    }
  Pmm=P*0.0075;
  int8_t Tdig= T/100;
  float   height =  (-log(P/ start_height))*(((T/100)+273)/0.0341593);
    

  char temp_str[20] = {0,};
snprintf(temp_str, 3, "%d", Tdig);
ST7789_WriteString(23, 1, temp_str, Font_11x18, YELLOW, 0xBDF8);
  Pmm=P*0.0075;
/*
snprintf(temp_str, 20, "H:%dm", 22);
ST7789_WriteString(180, 1, temp_str, Font_11x18, YELLOW, 0xBDF8);
*/
snprintf(temp_str, 20, "%dmm", Pmm);
ST7789_WriteString(23, 20, temp_str, Font_11x18, YELLOW, 0xBDF8);
 ...
 float lsb=0;
  ...
  if(height<0) 
  {
    ...
    msb=fabs(msb);
    sign_height=1;
    ST7789_WriteString(23, 40,"-", Font_11x18, YELLOW, 0xBDF8);
  }
  else 
  {
  sign_height=0;
  ...
  uint8_t ui_lsb= (uint8_t) (lsb*10);
  uint16_t ui_msb= (uint16_t) msb;
  snprintf(temp_str, 20, "%d.%dm",ui_msb, ui_lsb);
ST7789_WriteString(34, 40, temp_str, Font_11x18, YELLOW, 0xBDF8);  

 snprintf(temp_str, 20, "%d",steeep);
ST7789_WriteString(67, 220, temp_str, Font_11x18, YELLOW, 0xBDF8); 

}

void ST7789_RTC_Update()
{
  char temp_str[20] = {0,};
  if(rtc_H<10  &&  rtc_M<10  )  snprintf(temp_str, 20, "0%d:0%d",rtc_H, rtc_M);
  if(rtc_H>=10 &&  rtc_M<10  )  snprintf(temp_str, 20, "%d:0%d",rtc_H, rtc_M);
  if(rtc_H<10  &&  rtc_M>=10 )  snprintf(temp_str, 20, "0%d:%d",rtc_H, rtc_M);
  if(rtc_H>10  &&  rtc_M>10 )  snprintf(temp_str, 20, "%d:%d",rtc_H, rtc_M);
 

  //snprintf(temp_str, 20, "%d:%d",rtc_H, rtc_M);
  ST7789_WriteString(80, 105, temp_str, Font_16x26, YELLOW, 0xBDF8);
